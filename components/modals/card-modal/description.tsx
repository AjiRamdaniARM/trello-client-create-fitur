...
export const Description = ({ data }: DescriptionProps) => {
  ...
  const handleSendMessage = () => {
    ...
    const message = { id: Date.now(), text: newMessage, isSent: true };
  ...
  const handleReceiveMessage = (text: string) => {
    const message = { id: Date.now(), text, isSent: false };
    const updatedMessages = [...messages, message];
    setMessages(updatedMessages);
    localStorage.setItem("chatMessages", JSON.stringify(updatedMessages));
  };
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      handleReceiveMessage("Ini pesan dari pengguna lain.");
    }, 5000);
  ...
  <div className="flex flex-col">
    ...
    <div className="mt-4 bg-neutral-100 p-4 rounded-lg max-h-80 overflow-y-auto">
      {messages.map((msg) => (
        <div
          key={msg.id}
